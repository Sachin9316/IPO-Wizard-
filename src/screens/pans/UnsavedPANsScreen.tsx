...
export const UnsavedPANsScreen = () => {
    ...
    const [pans, setPans] = useState<PANData[]>([
        { id: '1', panNumber: 'GHIJK7890K' },
        { id: '2', panNumber: 'BCDEF2345L' },
        { id: '3', panNumber: 'STUVW6789M' },
    ]);

    const handleAddPAN = (data: { panNumber: string }) => {
        if (editingPAN) {
            setPans(pans.map(pan =>
                pan.id === editingPAN.id
                    ? { ...pan, panNumber: data.panNumber }
                    : pan
            ));
            setEditingPAN(null);
        } else {
        ...
        <View style={[styles.panCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <View style={styles.panCardContent}>
                ...
                <View style={styles.panInfo}>
                    ...
                    <Text style={[styles.panSubtext, { color: colors.text, opacity: 0.5 }]}>XXXX</Text>
                </View>
            </View>
            ...
            {pans.length === 0 ? (
                <View style={styles.emptyContainer}>
                    ...
                    <Text style={[styles.emptySubText, { color: colors.text, opacity: 0.4 }]}>
                        Tap the + button to add a temporary PAN
                    </Text>
                </View>
            ) : (
                <FlatList
